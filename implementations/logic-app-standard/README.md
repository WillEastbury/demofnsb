...
This sample implements the same asynchronous flow without application code:
...
`AttributeStatusWriter\workflow.json`:
...
Create the `AttributeStatus` table before starting the workflow. Use the Logic
Apps extension's **Run/Debug** command to start the local Functions-based
workflow runtime.

## Message contract

The defaults expect top-level `siteId` and `attributeId` fields. Change these
settings to match the existing contract:
...
`samples\attribute-status-message.json`.
...
changing identifiers. Add a transformation action only if the CC API applies
the same deterministic key encoding.
